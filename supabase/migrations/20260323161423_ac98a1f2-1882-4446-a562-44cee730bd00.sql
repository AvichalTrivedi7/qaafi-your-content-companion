...
-- Update submit_offer: recalculate is_locked after sibling release
...
BEGIN
  ...
  IF _action = 'accept' THEN
    ...
    IF sibling_release > 0 THEN
      new_reserved := GREATEST(0, (SELECT reserved_quantity FROM public.rfqs WHERE id = neg.rfq_id) - sibling_release);
      UPDATE public.rfqs
...
-- Update accept_negotiation: recalculate is_locked after sibling release
...
BEGIN
  ...
  -- Cancel siblings and release their meters with lock recalculation
  SELECT COALESCE(SUM(negotiation_quantity), 0) INTO sibling_release
  FROM public.negotiations
  WHERE rfq_id = neg.rfq_id AND id != _negotiation_id AND status IN ('open', 'offer_made', 'counter_offered');
